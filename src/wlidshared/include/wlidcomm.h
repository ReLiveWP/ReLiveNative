...
#define PERSONALISATION_VALUE "MicrosoftDeviceGuidSALT"
#define CONTAINER_NAME "IDENTITYCRL_CERT_CONTAINER"
...
typedef struct
{
    DWORD_PTR hIdentity;
    WCHAR szServiceTarget[256];
    WCHAR szServicePolicy[64];
    DWORD dwTokenRequestFlags;
} IOCTL_AUTH_IDENTITY_TO_SERVICE_ARGS, *PIOCTL_AUTH_IDENTITY_TO_SERVICE_ARGS;
...
} IOCTL_AUTH_IDENTITY_TO_SERVICE_RETURN, *PIOCTL_AUTH_IDENTITY_TO_SERVICE_RETURN;

#define IOCTL_WLIDSVC_AUTH_IDENTITY_TO_SERVICE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80C, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct
{
    DWORD_PTR hIdentity;
    WCHAR szCredType[64];
} IOCTL_PERSIST_CREDENTIAL_ARGS, *PIOCTL_PERSIST_CREDENTIAL_ARGS;

#define IOCTL_WLIDSVC_PERSIST_CREDENTIAL CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80D, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct
{
    WCHAR szCredType[64];
} IOCTL_ENUM_IDENTITIES_WITH_CACHED_CREDENTIALS_ARGS, *PIOCTL_ENUM_IDENTITIES_WITH_CACHED_CREDENTIALS_ARGS;
...
} IOCTL_ENUM_IDENTITIES_WITH_CACHED_CREDENTIALS_RETURN, *PIOCTL_ENUM_IDENTITIES_WITH_CACHED_CREDENTIALS_RETURN;

#define IOCTL_WLIDSVC_ENUM_IDENTITIES_WITH_CACHED_CREDENTIALS CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80E, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct
{
    DWORD_PTR hServerHandle;
} IOCTL_CLOSE_ENUM_IDENTITIES_HANDLE, *PIOCTL_CLOSE_ENUM_IDENTITIES_HANDLE;

#define IOCTL_WLIDSVC_CLOSE_ENUM_IDENTITIES_HANDLE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80F, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct
{
    DWORD_PTR hIdentity;
    DWORD dwProperty;
    WCHAR szProperty[256];
} IOCTL_SET_IDENTITY_PROPERTY_ARGS, *PIOCTL_SET_IDENTITY_PROPERTY_ARGS;

#define IOCTL_WLIDSVC_SET_IDENTITY_PROPERTY CTL_CODE(FILE_DEVICE_UNKNOWN, 0x810, METHOD_BUFFERED, FILE_ANY_ACCESS)

typedef struct
{
    DWORD dwFlags;
    WCHAR szAdditionalParams[256];
    BOOL bNeedsCert;
} IOCTL_GET_DEVICE_ID_ARGS, *PIOCTL_GET_DEVICE_ID_ARGS;

typedef struct
{
    WCHAR szDeviceId[512];
    BYTE bDeviceCertThumb[20];
} IOCTL_GET_DEVICE_ID_RETURN, *PIOCTL_GET_DEVICE_ID_RETURN;

#define IOCTL_WLIDSVC_GET_DEVICE_ID CTL_CODE(FILE_DEVICE_UNKNOWN, 0x811, METHOD_BUFFERED, FILE_ANY_ACCESS)
