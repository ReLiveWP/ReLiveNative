    ...
    int init_db(void)
    {
        ...
        if (rc == SQLITE_ROW)
        {
            ...
            if (version < CURRENT_SCHEMA_VERSION)
            {
                ...
                sqlite3_bind_text(update_stmt, 1, std::to_string(CURRENT_SCHEMA_VERSION).c_str(), -1, SQLITE_TRANSIENT);
                if ((code = sqlite3_step(update_stmt)) != SQLITE_DONE)
                {
        ...
        else
        {
            ...
            sqlite3_bind_text(insert_stmt, 1, std::to_string(CURRENT_SCHEMA_VERSION).c_str(), -1, SQLITE_TRANSIENT);
            if ((code = sqlite3_step(insert_stmt)) != SQLITE_DONE)
            {
